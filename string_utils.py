class StringUtils:
    ...
    def trim(self, string: str) -> str:
        ...
        """
        whitespace = " "
        while (string.startswith(whitespace)): #если строканачинается с пробела, то пробелы удаляются
    ...
    def starts_with(self, string: str, symbol: str) -> bool:
        """
        Возвращает `True`, если строкаstring, symbol начинается с заданного символа и `False` - если нет \n 
        Параметры: \n 
            `string` - строка для обработки \n
            `symbol` - искомый символ \n
